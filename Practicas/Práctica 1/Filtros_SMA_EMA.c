...
int main(void)
{
  /* USER CODE BEGIN 1 */
	uint16_t raw;
	char msg[10];
	float y=0, y_n=0, alpha=0.3, voltaje=0, suma=0;
	int i=0;
  /* USER CODE END 1 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  voltaje = ((float)raw)/4095*3.3;							    // Transforma el valor de raw y lo divide por el tamaño de bits (2^12-1)
	  	  	  	  	  	  	  	  	  	  	  	  	  	// multiplicado por el voltaje de referencia según el arreglo del circuito, en este caso 3.3
	  // Filtro EMA
    //y = (alpha * voltaje) + (1-alpha)*y_n;					
    y_n = y;													// Guarda el valor actual de y en una auxiliar para operar posteriormente con ella
